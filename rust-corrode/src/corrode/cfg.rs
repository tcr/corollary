...
pub fn addBlock<a>(label: Label, stmt: s, terminator: Terminator<c>) -> BuildCFGT<m, s, c, ()> {
    /*do*/ {
        modify(|st| { st {
...
pub fn buildCFG<a>(root: BuildCFGT<m, s, c, Label>) -> m<CFG<Unordered, s, c>> {
    /*do*/ {
...
pub fn depthFirstOrder<a>(CFG(start, blocks): CFG<k, s, c>) -> CFG<DepthFirst, s, c> {
    CFG(start_q, blocks_q)
}

pub fn flipEdges<a>(edges: IntMap::IntMap<IntSet::IntSet>) -> IntMap::IntMap<IntSet::IntSet> {
    IntMap::unionsWith(IntSet::union, /* Expr::Generator */ Generator)
}

pub fn hasMultiple<a>() -> bool {
    any((go(structureBody)))
}

pub fn mapBuildCFGT<a>() -> BuildCFGT<n, s, c, b> {
    mapStateT
}

pub fn newLabel<a>() -> BuildCFGT<m, s, c, Label> {
    /*do*/ {
        let old = get;
...
pub fn outEdges<a>(blocks: IntMap::IntMap<StructureBlock<s, c>>) -> IntSet::IntSet {
    IntSet::difference(IntSet::unions((__map!(successors)(IntMap::elems(blocks)))), IntMap::keysSet(blocks))
}

pub fn partitionMembers<a>(a: IntSet::IntSet, b: IntSet::IntSet) -> (IntSet::IntSet, IntSet::IntSet) {
    (IntSet::intersection(a, b), IntSet::difference(a, b))
}

pub fn prettyCFG<a>(fmtS: fn(s) -> Doc, fmtC: fn(c) -> Doc, CFG(entry, blocks): CFG<k, s, c>) -> Doc {
    vcat(__op_concat((__op_ne(text("start @".to_string()), text((show(entry))))), blocks_q))
}

pub fn prettyStructure<a>() -> Doc {
    vcat(__map!(go))
}

pub fn relooper<a>(entries: IntSet::IntSet, blocks: IntMap::IntMap<StructureBlock<s, c>>) -> Vec<Structure<s, c>> {
    {
...
pub fn relooperRoot<a>(CFG(entry, blocks): CFG<k, s, c>) -> Vec<Structure<s, c>> {
    relooper((IntSet::singleton(entry)))(IntMap::map((|BasicBlock(s, term)| { (s, fmap(GoTo, term)) }), blocks))
}

pub fn removeEmptyBlocks<a>(CFG(start, blocks): CFG<k, f<s>, c>) -> CFG<Unordered, f<s>, c> {
...
pub fn simplifyStructure<a>() -> Vec<Structure<s, c>> {
    foldr(go, vec![], __map!(descend))
}

pub fn structureCFG<a>(mkBreak: fn(Option<Label>) -> s, mkContinue: fn(Option<Label>) -> s, mkLoop: fn(Label) -> fn(s) -> s, mkIf: fn(c) -> fn(s) -> fn(s) -> s, mkGoto: fn(Label) -> s, mkMatch: fn(Vec<(Label, s)>) -> fn(s) -> s, cfg: CFG<DepthFirst, s, c>) -> (bool, s) {
    (hasMultiple(root), foo(vec![], mempty, root))
}

pub fn successors<a>((_, term): StructureBlock<s, c>) -> IntSet::IntSet {
