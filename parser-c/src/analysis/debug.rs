...
pub fn prettyAssocsWith(label: String,
                        prettyKey: fn(k) -> Doc,
                        prettyVal: fn(v) -> Doc,
                        theMap: Vec<(k, v)>)
                        -> Doc {
    ...
    let prettyEntry = |(k, v)| {
        __op_doc_conat(prettyKey(k),
                       __op_doc_conat(text(" ~> ".to_string()), prettyVal(v)))
    };

    __op_line_something(text(label), nest(8, (vcat(__map!(prettyEntry, theMap)))))
}

pub fn globalDeclStats(file_filter: fn(FilePath) -> bool,
                       gmap: GlobalDecls)
                       -> Vec<(String, isize)> {
    ...
    vec![("Enumeration Constants".to_string(), Map::size(enumerators)),
