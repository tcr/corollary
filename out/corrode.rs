        ...
        fmap(wrapping)(match op {
                ...
                CAddOp => {
                    match (toPtr(lhs), toPtr(rhs)) {
                        (Just(ptr), _) => {
                            return((offset(ptr, rhs)))
                        },
                        (_, Just(ptr)) => {
                            return((offset(ptr, lhs)))
                        },
                        _ => {
    ...
    fn interpretBlockItem(__0: CSourceBuildCFGT) -> CSourceBuildCFGT {
        ...
        CBlockDecl(decl)(next) => {
            /* do */ {
                ...
                let (rest, end) = next;
                return((__op_addadd(decl_q, rest), end))
            }
        },
        item(_) => {
        ...
        _(expr, <todo>, CIndex(lhs, rhs, _)) => {
            /* do */ {
                ...
                match (resultType(lhs_q), resultType(rhs_q)) {
                    (IsArray(mut, _, el), _) => {
                        return((subscript(mut, el, (result(lhs_q)), rhs_q)))
                    },
                    (_, IsArray(mut, _, el)) => {
                        return((subscript(mut, el, (result(rhs_q)), lhs_q)))
                    },
                    _ => {
                        /* do */ {
...
;
                    ...
                    ty(:, _) => {
                        return((literalNumber(ty, (Rust_LitInt(v, repr_q)))))
                    },
                })
                },
                CFloatConst(CFloat(str), _) => {
                    match span((Operator("notElem")("fF".to_string())), str) {
                        (v, "") => {
                            return((literalNumber((IsFloat(64)), (Rust_LitFloat(v)))))
                        },
                        (v, [_]) => {
                            return((literalNumber((IsFloat(32)), (Rust_LitFloat(v)))))
                        },
                        _ => {
                ...
                CStrConst(CString(str, False), _) => {
                    ...
                    resultMutable: Rust_Immutable,
                    result: Rust_Deref((Rust_Lit((Rust_LitByteStr((__op_addadd(str, "\u{0}".to_string())))))))
                    })
                },
                _ => {
                    unimplemented(expr)
    ...
    fn interpretFunction((@(CFunDef(specs, declr), (CDeclr(mident, _, _, _, _))(argtypes, body, _))): EnvMonad) -> EnvMonad {
        /* do */ {
            ...
            let (attrs, vis) = match storage {
                    Nothing => {
                        return((vec![Rust_Attribute("no_mangle".to_string())], Rust_Public))
                    },
                    Just(CStatic(_)) => {
                        return((vec![], Rust_Private))
                    },
                    Just(s) => {
    ...
    fn interpretStatement(__0: CSourceBuildCFGT) -> CSourceBuildCFGT {
        ...
        CExpr(Just(expr), _)(next) => {
            /* do */ {
                ...
                let (rest, end) = next;
                return((__op_addadd(resultToStatements(expr_q), rest), end))
            }
        },
        ...
        IsPtr(mut, to) => {
            {
                    let Rust_TypeName = |to_q| {
                        toRustType(to, in, Rust_TypeName, (__op_addadd(rustMut(mut), to_q)))
                    };
            }
        },
        IsArray(_, size, el) => {
...
mod Language_Rust_Idiomatic {
    ...
    fn tailBlock(__0: Rust.Block) -> Rust.Block {
        match (__0) {
        Rust_Block(b, Just(<todo>)) => {
            Rust_Block(b, e)
        },
        Rust_Block(<todo>, Nothing) => {
            Rust_Block(b, e)
        },
        b => {
            b
